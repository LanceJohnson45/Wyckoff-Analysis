        ...
        try:
            with connect_postgres() as conn, conn.cursor() as cur:
                cur.execute(
                    ...
                    from public.index_constituents_snapshot
                    where market = %s and index_code = %s
                    limit 1
                    """,
                    (str(market or "").strip().lower(), str(index_code or "").strip().lower()),
                )
                row = cur.fetchone()
            if not row:
                return None
...
def upsert_index_constituents_snapshot(
    ...
    if postgres_enabled():
        try:
            ...
            upsert_rows(
                TABLE_INDEX_CONSTITUENTS_SNAPSHOT,
                [payload],
